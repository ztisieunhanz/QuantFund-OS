...
A-01 StrategyEligibility, the partial A-02 `ActionDecision` contract, the partial A-03 lifecycle methodology, and A-04 Step 1/6 producer provenance are checkpointed. A-04 Step 2/6 adds a derived canonical replay valuation witness from exact completed 1H closes (`candle-open timestamp + 1H = decisionTime`) and exact Risk valuation binding without changing ledger or execution authority. Its numerical weight reconciliation is not lifecycle comparison policy. `WAIT` remains the only constructible action; durable execution-target lifecycle state, comparison tolerance, pending/partial/completed execution assessment, and all non-WAIT derivation remain deferred to A-04 Steps 3-6. This does not alter the M14 authority chain or authorize execution.
