...
- **Gate Statuses**:
  ...
  - **M14 / A-02** (ActionDecision Contract): **COMPLETE AS PARTIAL CONTRACT** at `761353b59f092ac3511781f9de5e0538264bd755`; only fail-closed `WAIT` is constructible
  - **M14 / A-03** (Action Lifecycle Methodology + Contract): **COMPLETE AS PARTIAL CONTRACT** at `f70085027d2da235ca9647db0b4cb1086351a7f6`; all six meanings are frozen while non-WAIT numeric derivation and execution lifecycle remain deferred to A-04
  - **M14 / A-04 Step 1/6** (Producer Provenance Foundations): **COMPLETE** at `da599a325fc8c20f9d32e6ba091a14e9861e1d01`; Signal, Permission, Risk, and Omega target outputs carry deterministic reconstructable producer provenance without enabling non-WAIT actions
  - **M14 / A-04 Step 2/6** (Canonical Portfolio Valuation Witness): **COMPLETE** at `aa8fb7379b5dcaf9bc97d0104c805b3a55b6aaa0`; canonical LIVE replay derives immutable NAV/current-weight evidence from the execution account and exact held-asset bars whose candle-open timestamp plus 1H equals the decision boundary, while unprovable synthetic/standalone-live paths remain explicitly unbound
  - **M14 / A-04 Step 3/6 + R1/R2** (Execution Assessment + Target Lineage): **COMPLETE** at `d63568f0c8a6094884b63ba6b60d531fc534023e`; decision-bound evidence is explicitly distinct from self-reconstructing execution-bound evidence, canonical LIVE next-open replay transiently carries target/root bindings through actual fills and post-fill reconciliation, retained execution evidence is always revalidated, no lifecycle sidecar is added to `DecisionState`, missing execution evidence fails closed, and lifecycle roots use normalized economic allocation content rather than explanatory metadata
  - **M14 / A-04 Step 4/6 + R1** (Canonical ActionDecision Classification): **COMPLETE** at `a3ce0b596e4c4140751c4e1cf7fd68aa0053abff`; all six frozen long-only actions are derived only from integrity-validated Step 2/3 evidence and the canonical execution-planner USD materiality policy, while invalid or incompatible evidence fails closed to `WAIT`
  - **M14 / A-04 Step 5/6 + R1/R2** (Durable Lifecycle Persistence + Replay Integration): **COMPLETE** at `a00d0e7c86ad5fe86ac5673174495eff1713a70d`; one separately versioned, non-authoritative checkpoint persists the minimum ordered lifecycle witness, cold-revalidates every predecessor transition, binds the decision plane through Risk's canonical valuation identity plus exact mark-invariant ledger inventory, and reconstructs rather than persists `ActionDecision`
  - **M14 / A-04 Step 6/6** (Closure Audit and Gate Completion): **IMPLEMENTED — INDEPENDENT REVIEW PASSED; EXACT-SHA CI PENDING**; cross-step audit and invariant verification proves single authority paths for target weights (Omega), fills (ExecutionEngine), accounting (canonical ledger), and executable prices (`BacktestDataset.assetBars`), downstream-only and observational ActionDecision behavior, fail-closed cold persistence and legacy migration, and unmodified replay economics without introducing any additional economic authority
- **Validation Baseline**:
  - `npm run build`: **PASS**
...
- **Current Documented Phase**: M13B, M13C, and M13D are **COMPLETE**. M14/A-01, partial A-02, partial A-03, and M14/A-04 Steps 1-5 are **COMPLETE** (Step 5 at `a00d0e7c86ad5fe86ac5673174495eff1713a70d`). Step 6 closure audit is implemented with independent review passed; A-04 closure and checkpointing remain pending exact-SHA CI. M15 has not started. No research, eligibility, ActionDecision, lifecycle, persistence, provenance, valuation-witness, execution-assessment, or target-lineage component grants predictive, paper-action, Permission, Risk, allocation, target-weight, execution, or accounting authority. Baseline replay, execution, and accounting parity remain strictly preserved.
...
1. **Single Persisted Decision Artifact**: `latestDecision` remains the sole decision/accounting artifact persisted in `localStorage` under key `quant_paper_engine_state`; A-04 Step 5 adds one separately typed non-authoritative lifecycle evidence checkpoint, not a second ledger or decision authority.
...
- A-02's versioned immutable contract records the intended `WAIT` / `ENTER` / `ADD` / `HOLD` / `REDUCE` / `EXIT` vocabulary but currently derives only `WAIT`. The ledger exposes units/cash rather than an identity-bearing current-weight snapshot, and the existing target object is not yet paired with reviewed target provenance, gate bindings, valuation time, and pending-execution semantics. A-02 therefore marks these sources unavailable and rejects caller-authored substitutes.
- A-03 freezes the conceptual long-only meanings and explicitly distinguishes fail-closed `WAIT` from proven positive/equal-target `HOLD`. A-04 Step 1 adds deterministic producer provenance for Signal, Permission, Risk, and Omega target outputs. Step 2 adds an immutable derived witness over the canonical replay execution account plus exact held-asset CLOSE marks with `bar.timestamp + 1H = decisionTime`; it mechanically derives NAV/current weights and binds Risk when that evidence is available. Its `1e-12` sum check only reconciles floating-point weights and cannot derive an action. Step 3/R1 extracts the ExecutionEngine's existing USD minimum-rebalance semantics into one immutable planner used by execution and pre/post assessments. Decision-bound evidence cannot claim a fill. Separate execution-bound evidence reconstructs canonical next-open execution, validates target/root-bound fill identities, binds pre/post accounts and the post-fill valuation, and classifies satisfied, partial, outstanding, or invalid. Step 3/R2 wires that proof into canonical LIVE replay using transient in-memory state: the pending target reaches Execution with its exact target/root binding, the actual result is reconstructed against the post-fill valuation, and lifecycle evidence is reconciled outside `DecisionState`. Every retained execution assessment is independently revalidated for artifact, root, economic-target, and temporal consistency; historical target-decision identity is preserved rather than rewritten as a later reaffirmation. Step 4/R1 derives `WAIT`, `ENTER`, `ADD`, `HOLD`, `REDUCE`, and `EXIT` from the validated active lifecycle's latest canonical valuation/target relationship, reusing the Step 3 planner threshold exactly and rejecting forged, stale, invalid, or contradictory evidence to `WAIT`. Step 5/R2 stores one versioned lifecycle evidence checkpoint separately from `DecisionState`, including the ordered prior witness needed to cold-revalidate every Step 3 transition. The lifecycle PIT valuation and end-of-bar `DecisionState` accounting mark may legitimately differ in NAV and unrealized PnL; Risk's validated canonical valuation identity is the decision-plane bridge, while cash and mark-invariant position inventory are compared exactly without creating a second ledger. Semantic identities provide deterministic reconstruction, not secret-backed authenticity. Legacy v1 state preserves only independently valid `DecisionState`; malformed or unknown lifecycle persistence cannot create a non-`WAIT` action. Step 6 closure audit is implemented with independent review passed, proving single authority paths, fail-closed cold persistence and migration, downstream-only ActionDecision observation, and unmodified replay economics without introducing any additional economic authority. A-04 closure remains pending exact-SHA CI.
...
- **M14 and later**: A-01, partial A-02, partial A-03, and A-04 Steps 1-5 are complete (Step 4/R1 at `a3ce0b596e4c4140751c4e1cf7fd68aa0053abff`, Step 5/R2 at `a00d0e7c86ad5fe86ac5673174495eff1713a70d`). Step 6 closure audit is implemented with independent review passed; A-04 closure remains pending exact-SHA CI. M15 has not started. D-C training/calibration infrastructure is not required unless a future approved methodology introduces genuine TRAIN-only fitting.
...
**Status**: **COMPLETE AS PARTIAL CONTRACT** at `761353b59f092ac3511781f9de5e0538264bd755`
...
The contract fixes 1H as the executable domain, keeps derived 4H/1D context explanatory and unbound, preserves long-only v1, and makes every Permission/Risk/allocation/target/execution/accounting authority flag false. Execution continues to consume `TargetPortfolioWeight` directly.

### M14 / A-03 — Action Lifecycle Methodology + Contract

**Status**: **PARTIAL CONTRACT IMPLEMENTED — INDEPENDENT REVIEW PENDING / NOT CHECKPOINTED**

A-03 freezes the long-only lifecycle vocabulary without constructing a second action engine. `ENTER` means zero current exposure with a materially positive authorized target; `ADD` means positive exposure with a materially higher target; `HOLD` means positive exposure materially equal to target; `REDUCE` means a lower but still positive target; and `EXIT` means positive exposure with a zero target. These five meanings are methodology only and remain non-constructible. `WAIT` remains the sole constructible result and means that no authorized transition is currently provable; it is not an alias for `HOLD`.

The policy deliberately defines no epsilon. The execution engine's default USD rebalance threshold is an execution-detail threshold, not a canonical portfolio-weight equality policy. The replay engine keeps `pendingRebalance` as internal transient state and clears it after the next-bar attempt; `DecisionState` does not preserve outstanding, partial, completed, repeated, or stale target lineage. A-03 records these gaps explicitly and defers numeric comparison and lifecycle derivation to A-04. StrategyEligibility remains admission only, Omega remains sole target-weight authority, Execution continues consuming `TargetPortfolioWeight` directly, and the ledger remains sole current-state authority.
