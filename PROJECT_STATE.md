...
- **Gate Statuses**:
  ...
  - **M14 / A-04 Step 5/6 + R1/R2** (Durable Lifecycle Persistence + Replay Integration): **IMPLEMENTED — INDEPENDENT REVIEW PENDING**; one separately versioned, non-authoritative checkpoint persists the minimum ordered lifecycle witness, cold-revalidates every predecessor transition, binds the decision plane through Risk's canonical valuation identity plus exact mark-invariant ledger inventory, and reconstructs rather than persists `ActionDecision`
- **Validation Baseline**:
  - `npm run build`: **PASS**
...
- **M14 and later**: A-01, partial A-02, partial A-03, and A-04 Steps 1-4 are checkpointed. A-04 Step 5/R2 durable lifecycle recovery is implemented for independent review; `ActionDecision` is reconstructed rather than persisted, and A-04 closure remains deferred to Step 6. D-C training/calibration infrastructure is not required unless a future approved methodology introduces genuine TRAIN-only fitting.
