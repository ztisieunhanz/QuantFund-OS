...
A-04 Step 4/R1 implements canonical product-facing classification over the validated Step 3 active lifecycle without inserting `ActionDecision` into the authority chain. Newly authorized or reaffirmed targets classify from the latest decision-bound Step 2 valuation and Omega target; execution transitions classify from the validated post-fill Step 2 valuation and retained target. Materiality is exactly the Step 3/ExecutionEngine USD rebalance policy, including its default USD 50 threshold, exact-threshold behavior, custom thresholds, and the rule that exact zero delta is never executable. `HOLD` requires proven positive current and target exposure with no executable rebalance; zero-to-zero and any unavailable, forged, stale, temporally mismatched, invalid, or contradictory evidence remain `WAIT`. The artifact independently reconstructs from embedded canonical evidence, binds valuation/target/planner/root/Permission/Risk provenance identities, and grants no Permission, Risk, allocation, target, execution, ledger, accounting, predictive, or paper-action authority. Step 4 is complete at `a3ce0b596e4c4140751c4e1cf7fd68aa0053abff` and is not consumed by Execution.

A-04 Step 5/R2 adds one versioned `DurableTargetLifecycleCheckpoint` alongside, not inside, persisted `DecisionState`. The checkpoint carries the terminal Step 3 lifecycle, the minimum ordered prior lineage needed to re-prove every root-to-terminal transition after cold JSON restore, the action asset, exact `DecisionState` identity, and only non-authority flags. Every retained lifecycle artifact is cold-reconstructed and each predecessor relationship is revalidated with the existing Step 3 transition validator; an opaque predecessor or superseded-root hash is never accepted as proof by itself.

At the same API timestamp, two deliberately different marking boundaries coexist. The lifecycle's canonical valuation is the PIT decision-boundary witness using the exact completed close available at `t` (`bar.timestamp + 1H = t`), while `DecisionState.nav` and position unrealized PnL retain the existing end-of-bar accounting mark. Equality of NAV, unrealized PnL, or the full mark-dependent account identity is therefore neither required nor expected. The canonical bridge is the independently validated `DecisionState.risk.provenance.valuationIdentity`, which must be bound to and equal the lifecycle current valuation identity; exact cash and the mark-invariant position inventory (asset set, asset id, quantity, entry price, side, and status) are also compared directly. This projection is evidence only and does not become a ledger or valuation authority. Rehydration then derives and validates `ActionDecision`; the action itself is never persisted. Semantic identities provide deterministic integrity and reconstruction, not secret-backed authenticity against arbitrary wholesale persistence rewriting. Store schema v2 migrates valid v1 `latestDecision` conservatively with lifecycle/action unavailable and rejects unknown versions. Invalid lifecycle persistence is discarded without fabricating a root or action, reset clears both checkpoint and derived action, and canonical replay economics remain unchanged. This is recovery visibility, not an incremental execution-resume engine, second ledger, or authority. Step 5 is complete at `a00d0e7c86ad5fe86ac5673174495eff1713a70d`.

A-04 Step 6 closure audit is complete at `ddf836503b13e0c70e30c5fe78710a6cc3682592`, validated by exact-SHA GitHub Actions CI run `36250600167` (`Test and build`: success). It proves that Steps 1–5 collectively satisfy the frozen ActionDecision and lifecycle contracts with exactly one authority path for target weights (Omega), fills (ExecutionEngine), accounting (canonical ledger), and executable prices (`BacktestDataset.assetBars`), downstream-only and observational ActionDecision behavior, fail-closed cold persistence and legacy migration, and unmodified replay economics without introducing any additional economic authority. M14/A-04 is closed at that checkpoint; M15 is next and has not started.
...
Derived 4H/1D research context, the stateless ResearchRule/combinator foundation, bounded stateful sequence/persistence primitives, deterministic Hypothesis Registry governance, PIT-safe research feature construction, the research-only shadow observation harness, fixed-trial held-out OOS evidence aggregation, D-B-R1 robustness-family preregistration, descriptive shared-OOS D-B evaluation, and the D-D research Evidence Registry are implemented without connection to trading behavior. D-A is checkpointed at `2e3474f8640cf9abbfaa70388d3fcaec137aa9be`; D-B-R1 is checkpointed at `5dfa01c0fbec51893293832c4ca7599c7b8f6490`; D-B is checkpointed at `61722d9639a49083ad052fccf5a286caaf95ad1a`; the D-C not-required audit is checkpointed at `14ac61c888e5ba5b34070a455cdda98c005af31e`; D-D/D-D-R1 are checkpointed at `901c687a79a01b663d33c539fb6a6a9960c5430a`; A-01 StrategyEligibility is checkpointed at `d74adfd2c5b062b182a51c38922605b04b1901ee`; A-02 is checkpointed at `761353b59f092ac3511781f9de5e0538264bd755`; A-03 is checkpointed at `f70085027d2da235ca9647db0b4cb1086351a7f6`; A-04 Step 1 is checkpointed at `da599a325fc8c20f9d32e6ba091a14e9861e1d01`; A-04 Step 2/R1 is checkpointed at `aa8fb7379b5dcaf9bc97d0104c805b3a55b6aaa0`; A-04 Step 3/R1/R2 is checkpointed at `d63568f0c8a6094884b63ba6b60d531fc534023e`; A-04 Step 4/R1 is checkpointed at `a3ce0b596e4c4140751c4e1cf7fd68aa0053abff`; A-04 Step 5/R2 is checkpointed at `a00d0e7c86ad5fe86ac5673174495eff1713a70d`; and A-04 Step 6 closure is checkpointed at `ddf836503b13e0c70e30c5fe78710a6cc3682592` with exact-SHA CI run `36250600167` successful. No hypothesis has established predictive validity or approval for paper action, and baseline replay, execution, and accounting parity remain strictly preserved. M15 has not started.
